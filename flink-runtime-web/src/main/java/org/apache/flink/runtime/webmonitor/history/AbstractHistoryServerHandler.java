...
public abstract class AbstractHistoryServerHandler<T>
    ...
    protected final ArchiveStorage<T> archiveStorage;

    protected AbstractHistoryServerHandler(ArchiveStorage<T> archiveStorage, File rootPath)
            throws IOException {
    ...
    protected void respondToRequest(ChannelHandlerContext ctx, RoutedRequest routedRequest)
        ...
        if (!requestPath.contains(".")) { // we assume that the path ends in either .html or .js
            ...
            T resource = loadResource(requestPath);
